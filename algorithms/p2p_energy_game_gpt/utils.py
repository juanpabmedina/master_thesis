...
def evaluate_generator(agent_G, agent_C, kwargs, evaluate_param):
...
def evaluate_consumer(agent_G, agent_C, kwargs, evaluate_param):
    ...
    for _ in range(total_steps):
        ...
        action_C = agent_C.select_action(state, evaluate=evaluate_param)
        if agent_G is None:
            action_G = random.choice(CONSUMER_ACTIONS)
        else:
            action_G = agent_G.select_action(state, evaluate=evaluate_param)
...
def train_generator(mode, kwargs):
    ...
    agent_G = MinimaxQAgent(player_id='G', actions=GENERATOR_ACTIONS, opponent_actions=CONSUMER_ACTIONS)

    if mode == 'MR':
        agent_C = None  # Consumer acts randomly
    elif mode == 'MM':
        agent_C = MinimaxQAgent(player_id='C', actions=CONSUMER_ACTIONS, opponent_actions=GENERATOR_ACTIONS)
    elif mode == 'MC':
...
def train_consumer(mode, kwargs):
    total_steps = kwargs.get('total_steps', 10000)
    eval_interval = kwargs.get('eval_interval', 100)
    init_gen_power = kwargs.get('init_gen_power', 0.3)
    init_con_price = kwargs.get('init_con_price', 2.0)
    ...
    if mode == 'MR':
        agent_G = None  # Consumer acts randomly
    elif mode == 'MM':
        agent_G = MinimaxQAgent(player_id='G', actions=GENERATOR_ACTIONS, opponent_actions=CONSUMER_ACTIONS)
    elif mode == 'MC':
        with open('trained_challenger.pkl', 'rb') as f:
            agent_G = pickle.load(f)
    else:
        raise ValueError("Mode must be 'MR', 'MM', or 'MC'.")
    ...
    with tqdm(total=total_steps, desc=f"Training Mode: {mode}") as pbar:
        ...
        while step_count < total_steps:
            ...
            action_C = agent_C.select_action(state)
            if agent_G is None:
                action_G = random.choice(GENERATOR_ACTIONS)
            else:
                action_G = agent_G.select_action(state)
            ...
            cumulative_reward += reward['consumer']
            step_count += 1
            pbar.update(1)
