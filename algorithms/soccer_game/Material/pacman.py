...
from rl_agent import PacmanAgent
...
rules = None
numGhosts = 2
layout_name = "smallGrid.lay"


pacmanAgent = PacmanAgent()
ghostAgents = [DirectionalGhost( i+1 ) for i in range( numGhosts )]
...
def reset_env(render=False):
    global rules, gameDisplay
    if(render):
        gameDisplay = graphicsDisplay.PacmanGraphics(1.0, frameTime=0.1)
    else:
        gameDisplay = textDisplay.NullGraphics()
    game = rules.newGame(mylayout, pacmanAgent, ghostAgents, gameDisplay, beQuiet, catchExceptions)
    state_mod, state = game.reset()
    return game, state_mod, state

def create_pacman_env(render=True):
    global rules, gameDisplay
