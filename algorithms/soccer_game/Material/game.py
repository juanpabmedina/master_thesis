...
class Game:
    ...
    def state_features (self, state):        
        ...
        ghost_position = state.getGhostPositions()[0]
        exist_food = (state.hasFood(1,1), state.hasFood(3,3))
        return (pacman_position, ghost_position, exist_food)
    ...
    def reset(self):
        ...
        simple_state = self.state_features(state)
        self.lastReward = 0.0
        return simple_state, state
    ...
    def step(self, action):
        ...
        for k in range(numAgents):
            ...
            # if(k==0):
            #     org_state = self.state.deepCopy()
            #     simple_state = self.state_features(self.state)
            #     reward = self.state.getScore() - self.lastReward
            #     self.lastReward = self.state.getScore()
        ...
        reward = self.state.getScore() - self.lastReward
        self.lastReward = self.state.getScore()
        return simple_state, reward, done, org_state
